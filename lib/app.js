...
lol.prototype._mentionTvbot = function(message, channel) {
      ...
      if(active === true && command !== 'none'){
        ...
        this._queryTv(tvObject, channel, command)
      }

}

lol.prototype._queryTv = function(tvObject, channel, command){
  ...
  function postShow(err, res, body){
    if (!err && res.statusCode == 200) {
     ...
     tvBot.postMessageToChannel(channel, res)

   } else {
     tvBot.postMessageToChannel(channel, "Sorry, I couldn't find that tvshow")
   }
  }
